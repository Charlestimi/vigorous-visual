...
```  
In non-`<script setup>` components, props are declared using the[ `props` ](https://vuejs.org/api/options-state.html#props)option:
...
```
More details: [Typing Component Props](https://vuejs.org/guide/typescript/composition-api.html#typing-component-props)
...
If you want to pass all the properties of an object as props, you can use [ `v-bind` ](https://vuejs.org/guide/essentials/template-syntax.html#dynamically-binding-multiple-attributes) without an argument (`v-bind` instead of `:prop-name`). For example, given a `post` object:
...
If using T[ype-based props declarations](https://vuejs.org/api/sfc-script-setup.html#type-only-props-emit-declarations) , Vue will try its best to compile the type annotations into equivalent runtime prop declarations. For example, `defineProps<{ msg: string }>` will be compiled into `{ msg: { type: String, required: true }}`.
