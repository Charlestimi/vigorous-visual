...
all Vue expressions by adding them to app.config.globalProperties.
...
A directive's job is to reactively apply updates to the DOM when the value of its expression changes.Take `v-if` as an example:  
...
![Template Syntax](/essentials/template-syntax.svg)
